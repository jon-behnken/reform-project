...
from powertools.main import generate_bill_data, generate_bill_fulltext, get_members_by_state, get_member_by_id
from flask import (
...
def show_bill(bill_id):
    ...
    govtrack_api_key = app.config['GOVTRACK_API_KEY']
    ...
    return render_template('bills.html', bill_title=bill_title, bill_intro_date=bill_intro_date, bill_history=bill_history, bill_fulltext=bill_fulltext, bill_sponsor=bill_sponsor_json)

@bill.route('/<bill_id>/history', methods=['GET', 'POST'])
def get_bill_history(bill_id):
    api_key = app.config['PROPUBLICA_API_KEY']
