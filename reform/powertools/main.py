...
proPublica_DataTable_url = 'https://api.propublica.org/congress/v1/bills/search.json?query={}&sort=date'
proPublica_bill_url = 'https://api.propublica.org/congress/v1/116/bills/{}.json'

proPublica_SENATE_member_url = 'https://api.propublica.org/congress/v1/members/{}/{}/current.json'
proPublica_SENATE_member_id_url = 'https://api.propublica.org/congress/v1/members/{}.json'
...
def generate_datatable_JSON(api_key, topicQueryString):
    ...
    response = requests.get(proPublica_DataTable_request_url, headers={'X-API-KEY': api_key})
    return json.dumps(response.json()['results'][0]['bills'])
...
def get_contact_form_url(member_id):
    ...
    yaml_file = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', 'yaml', member_file))
...
def get_members_by_state(api_key, member_state):
    senate_members_formatted_url = proPublica_SENATE_member_url.format('senate', member_state)
    response = requests.get(senate_members_formatted_url, headers={'X-API-KEY': api_key})
